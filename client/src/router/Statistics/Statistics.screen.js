...
const Statistics = () => {
	...
	useEffect(() => {
		(async () => {
			...
			setStatisticsData(await getData({ day: dateSelected, employeeId: employeeSelected }));
		})();
	}, []);
				...
				<div className="select-item">
					...
					<select onChange={(e) => setEmployeeSelected(e.target.value)}>
						...
						{statisticsData.employees &&
							statisticsData.employees.map((employee, index) => <option value={employee._id}>{employee.name}</option>)}
					</select>
					<span></span>
				</div>
				<div>
				...
				<table>
					...
					<tbody>
						{statisticsData.transactions &&
							statisticsData.transactions.map((item, index) => <TableRow index={index} {...item} />)}
					</tbody>
				</table>
			</div>
