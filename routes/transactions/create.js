...
router.post("/", async (req, res) => {
	try {
		let { servicesIds, employeeId } = req.body;

		//Validation
		if (servicesIds.length == 0 || !Array.isArray(servicesIds))
			return res.json({ status: false, message: "يجب اختيار خدمة واحدة علي الأقل" });
		...
		if (!employee) return res.json({ status: false, message: "الموظف الذي اخترته غير مسجل من قبل" });
		...
		for (let _id of servicesIds) {
			services.push(await ServiceModel.findOne({ _id: mongoose.Types.ObjectId(_id) }));
		}
